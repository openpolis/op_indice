...
def mps(request, group_by, type):
  ...
  return render_to_response("charts/parlamentari_%s.html" % group_by, { 'object_list': records, 'objects_count': sum(1 for rec in records), 'type': type, 'extraction_date': extraction_date, 'openparlamento_url': settings.OPENPARLAMENTO_URL })
